...
def check_and_download_s3_files(s3_bucket_url: str, local_data_lake_path: str, expected_files: list[str], folder: str = "data_lake") -> dict[str, bool]:
        ...
        with tempfile.NamedTemporaryFile(suffix=".zip", delete=False) as tmp_zip:
            ...
            if download_with_progress(s3_zip_url, tmp_zip_path, f"{folder}.zip"):
                ...
                try:
                    ...
                    download_results = {filename: True for filename in expected_files}
                except Exception as e:
                    print(f"✗ Error extracting {folder}.zip: {e}")
                    download_results = {filename: False for filename in expected_files}
                finally:
